...
describe('addTodoItem', () => {
  ...
  test('set error state of list store correctly if failed', () => {
    ...
    return store.dispatch(addTodoItem(sampleList._id, 'new item')).then(() => {
      const newState = store.getState();
...
describe('setDueTodoItem', () => {
  ...
  test('updates due date of to-do item correctly', () => {
    ...
    return store.dispatch(setDueTodoItem(sampleList._id, 'item2')).then(() => {
      const newState = store.getState();
...
describe('removeTodoItem', () => {
  ...
  test('removes to-do item correctly', () => {
    ...
    return store.dispatch(removeTodoItem(sampleList._id, 'item2')).then(() => {
      const newState = store.getState();
